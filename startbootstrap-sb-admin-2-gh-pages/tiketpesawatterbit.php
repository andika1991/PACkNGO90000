...
if (isset($_GET['invoice_id'])) {
    ...
    if ($stmt = $conn->prepare($query_select)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $pdf->SetSubject('Pesawat E-Ticket');
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $html .= '<p>Nama Maskapai: ' . $row['nama_vendor'] . '</p>';
                $html .= '<p>Kelas: ' . $row['kelas'] . '</p>';
                $html .= '</div>';

                $html .= '<div class="section">';
                $html .= '<p class="section-title">Informasi Keberangkatan dan Kedatangan</p>';
                $html .= '<p>Keberangkatan: ' . $row['bandara_keberangkatan'] . ' - ' . date('d/m/Y H:i', strtotime($row['waktu_keberangkatan'])) . '</p>';
                $html .= '<p>Kedatangan: ' . $row['bandara_kedatangan'] . ' - ' . date('d/m/Y H:i', strtotime($row['waktu_kedatangan'])) . '</p>';
                $html .= '<p>Nomor penerbangan: ' . $row['nomor_penerbangan'] . '</p>';
...
$html .= '</div>';
            ...
            // Output PDF
            $pdf->Output('tiket_pesawat.pdf', 'I');
        } else {
