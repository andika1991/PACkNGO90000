...
if(isset($_GET['id_jadwaltiketbus'])) {
    // Ambil ID tiket dari URL
    $id_tiket = $_GET['id_jadwaltiketbus'];
...
if(isset($_GET['id_pengguna']) && isset($_GET['id_jadwaltiketbus']) && isset($_GET['harga']) && isset($_GET['id_datapenumpang']) && isset($_GET['id_metode'])) {
    ...
    if(mysqli_query($conn, $query_insert)) {
        ...
        $query_select = "SELECT pa.invoice_id, pa.TIMEORDER, jtb.waktu_keberangkatan, jtb.waktu_kedatangan, jtb.terminal_keberangkatan, jtb.terminal_kedatangan, jtb.kelas, jtb.status_jadwal, vb.nama_vendor, dpb.jenis_kelamin, dpb.nama_lengkap, dpb.no_hp, dpb.email, dpb.kursi,dpb.nik, mp.nama_metode, mp.nomor_metode, mp.logo_metode, mp.Deksripsi_metode FROM pesanantiketbus AS pa JOIN jadwal_tiket_bus AS jtb ON pa.id_jadwaltiketbus = jtb.id_jadwaltiketbus JOIN vendor_bus AS vb ON jtb.id_vendorbus = vb.id_vendorbus JOIN datapenumpangbus AS dpb ON pa.id_datapenumpang = dpb.id_datapenumpang JOIN metodepembayaran AS mp ON pa.id_metode = mp.id_metode WHERE pa.invoice_id = '$invoice_id'";
