...
if (isset($_SESSION['email'])) {
    ...
    $jadwal_tiketkereta = mysqli_real_escape_string($conn, $_POST['id_jadwaltiketkereta']);

    // Ambil ID pengguna dari sesi
    $id_pengguna = $_SESSION['id_pengguna'];

    // Query untuk menyimpan data penumpang ke dalam database
    $query = "INSERT INTO `datapenumpangkereta`(`jenis_kelamin`, `nama_lengkap`, `no_hp`, `email`, `nik`, `id_pengguna`, `id_jadwaltiketkereta`)
    ...
    if (mysqli_query($conn, $query)) {
        ...
        header("Location: pilihpembayarankereta.php?id_pengguna=$id_pengguna&id_jadwaltiketkereta=$jadwal_tiketkereta&id_datapenumpang=$id_data_penumpang");
        exit();
    } else {
