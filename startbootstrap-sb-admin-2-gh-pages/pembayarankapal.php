...
if(isset($_GET['id_jadwaltiketkapal'])) {
    ...
    $id_tiket = $_GET['id_jadwaltiketkapal'];
...
if(isset($_GET['id_pengguna']) && isset($_GET['id_jadwaltiketkapal']) && isset($_GET['harga']) && isset($_GET['id_datapenumpang']) && isset($_GET['id_metode'])) {
    // Ambil data dari URL
    $id_pengguna = $_GET['id_pengguna'];
    $id_jadwaltiketkapal = $_GET['id_jadwaltiketkapal'];
    $harga = $_GET['harga'];
    ...
    $query_insert = "INSERT INTO pesanantiketkapal (bukti_bayar, status_pembayaran, id_datapenumpang, id_metode, invoice_id, TIMEORDER, id_pengguna, id_jadwaltiketkapal)
                     VALUES (NULL, 'Belum Bayar', $id_datapenumpang, $id_metode, '$invoice_id', '$timeorder', $id_pengguna, $id_jadwaltiketkapal)";
    ...
    if(mysqli_query($conn, $query_insert)) {
        ...
        $query_select = "SELECT pa.invoice_id, pa.TIMEORDER, jtb.waktu_keberangkatan, jtb.waktu_kedatangan, jtb.pelabuhan_keberangkatan, jtb.pelabuhan_kedatangan, jtb.kelas, jtb.status_jadwal, vb.nama_vendor, dpb.jenis_kelamin, dpb.nama_lengkap, dpb.no_hp, dpb.email, dpb.nik,dpb.nomor_kendaraan, mp.nama_metode, mp.nomor_metode, mp.logo_metode, mp.Deksripsi_metode FROM pesanantiketkapal AS pa JOIN jadwal_tiket_kapal AS jtb ON pa.id_jadwaltiketkapal = jtb.id_jadwaltiketkapal JOIN vendor_kapal AS vb ON jtb.id_vendor = vb.id_vendor JOIN datapenumpangkapal AS dpb ON pa.id_datapenumpang = dpb.id_datapenumpang JOIN metodepembayaran AS mp ON pa.id_metode = mp.id_metode WHERE pa.invoice_id = '$invoice_id'";
        ...
        if(mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "Nomor Kendaraan: " . $row["nomor_kendaraan"] . "<br>";
...
function generateInvoiceID($conn) {
    ...
    $query_last_invoice = "SELECT MAX(CAST(SUBSTRING(invoice_id, 9) AS UNSIGNED)) AS last_id FROM pesanantiketkapal";
...
?>
    ...
    <div id="uploadForm" style="display: none;">
        <form action="uploadkapal.php" method="POST" enctype="multipart/form-data">
