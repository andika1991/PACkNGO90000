...
if (isset($_SESSION['email'])) {
    ...
    $jadwal_tiketbus = $_GET['id_jadwaltiketkapal'];
    $id_data_penumpang = $_GET['id_datapenumpang'];
    
    // Query untuk mendapatkan harga tiket berdasarkan id_jadwaltiketbus
    $query_harga = "SELECT harga FROM jadwal_tiket_kapal WHERE id_jadwaltiketkapal = $jadwal_tiketbus";
...
echo "</div>";
        ...
        while($kategori_row = mysqli_fetch_assoc($result_kategori)) {
            ...
            while($row = mysqli_fetch_assoc($result_metode_pembayaran)) {
               ...
               echo '        <a href="pembayarankapal.php?id_pengguna=' . $id_pengguna . '&id_jadwaltiketkapal=' . $jadwal_tiketbus . '&harga=' . $harga . '&id_datapenumpang=' . $id_data_penumpang . '&id_metode=' . $row['id_metode'] . '" class="lanjutkan_pembayaran" id="link_' . $row['id_metode'] . '">Pilih Pembayaran</a>';
               echo '    </div>';
               echo '</div>';
